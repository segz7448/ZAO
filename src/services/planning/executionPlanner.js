...
- content: REQUIRED whenever action is "pc_fs_create_file" or "fs_create_file" - the FULL, complete, working text to write into that file (real code/config/text, not a placeholder or a description of what it should contain). For "pc_log_decision", put the WHY - the actual reasoning behind the decision named in target - here instead (this step reuses fs_create_file's target=path/content=text shape as target=decision/content=reasoning, rather than needing its own separate fields). Omit this field entirely for every other action.
...
FILE OUTPUT DEFAULTS TO THE PC, NOT THE PHONE: for any "files" domain step that writes a new file, use "pc_fs_create_file" - it lands on the PC backend's own disk, the same place Terminal/pc_run_tests/pc_git_* already operate, and where the person can explicitly pull a finished result to their phone afterward if they ask for it. Only use "fs_create_file" (the phone's own on-device storage, a separate machine Terminal can't see or build from) when the task explicitly names the phone/device ("save it to my phone", "put it in my downloads on the device") rather than just "create a file"/"make a list"/"write a report" with no device named.

TERMINAL ONLY GUARANTEES BASH AND PYTHON: terminal_pc_run_command steps can only assume bash and a Python interpreter exist - no other CLI (gh, aws, docker-compose, etc.) is guaranteed installed. Before planning a terminal_pc_run_command step around some other CLI, check whether a dedicated tool already does the same thing over a real API instead (e.g. listing GitHub repos is "github_list_repos", domain "github" - never a terminal step running \`gh repo list\`) - only fall back to terminal with bash/curl or python for things with no dedicated tool.
...
Keep each step small enough that if it fails on its own, the failure is easy to localize - don't bundle unrelated actions into one step. A step whose action is "pc_fs_create_file" or "fs_create_file" is USELESS without real content - never emit one without it.

Respond with ONLY a JSON object, no markdown fences, no commentary:
{
  "steps": [
    { "reasoning": "...", "domain": "files", "description": "...", "action": "pc_fs_create_file", "target": "path/to/file", "content": "...full file text...", "dependsOnStepIndex": null },
