/**
 * ZAO - Termux Terminal Tool
 *
 * Runs REAL shell commands via Termux's RUN_COMMAND API. This is one of
 * TWO terminal tools ZAO has - see also pcTerminalTool.js, which runs
 * commands on the person's PC via the backend server instead.
 *
 * ROLE: lightweight/fallback terminal. Intended for small, fast,
 * low-resource operations (git pull, npm install for small projects,
 * simple Python scripts, curl, ssh, file downloads) and as the automatic
 * fallback when the PC backend is unreachable or has no internet access -
 * see terminalRouter.js for how the model decides which of the two to use.
 * Heavy operations (APK builds, Docker, Android emulator, video
 * processing, Visual Studio builds) should go to the PC tool instead,
 * since the phone's CPU/RAM/thermal envelope is much more limited.
 *
 * ============================================================================
 ...
 *
 * SAFETY GATE: same commandSafety.js check as pcTerminalTool.js - see
 * that file's docstring for the full rationale. Applied here too since
 * a command routed to Termux is just as capable of wiping the phone's
 * own storage as a PC command is of wiping the PC's.
 *
